...
import { TR_SYSTEM_PROMPT, TR_GENERATION_PROMPT } from '../prompts/tr.prompt';
import { createGenerateDocxTool } from '../tools';

export const createTrStartGraph = (supabaseService: SupabaseService) => {
  return new StateGraph(TrAgentState)
    .addNode('load_template', async (state: TrAgentStateType) => {
      const { data } = await supabaseService.getAdminClient()
        ...
        .eq('agent_type', 'TR').eq('is_active', true).single();
      return { template: data };
    })
    .addNode('welcome', async (state: TrAgentStateType) => {
      if (!state.template) return {};
      const sections = state.template.sections.filter((s: any) => s.required);
      const message = new AIMessage(
        `Olá! Vou te ajudar a criar um **Termo de Referência** completo em conformidade com a Lei 14.133/2021.\n\n` +
        `Me fale sobre o que você precisa contratar. Pode incluir o objeto da contratação, quantidades, ` +
        `valores estimados, prazos, especificações técnicas — qualquer informação que tiver.\n\n` +
...
export const createTrReplyGraph = (
  ...
  return new StateGraph(TrAgentState)
    ...
    .addNode('process_message', async (state: TrAgentStateType) => {
...
O Termo de Referência já foi gerado com sucesso e está disponível para download.
...
Seja humano e prestativo.`,
      ...
      const extractionPrompt = `Você está coletando informações para elaborar um Termo de Referência conforme a Lei 14.133/2021.
...
- Nunca invente informações — baseie-se apenas no que o usuário forneceu ou está no documento.`;
      ...
      const aiResponse = await aiService.invoke(TR_SYSTEM_PROMPT, extractionPrompt);
      ...
      if (!response) {
        if (allSectionsCollected) {
          response = 'Ótimo! Coletei todas as informações necessárias. Vou gerar o Termo de Referência agora.';
        } else {
          ...
          response = next
            ...
            : 'Pode continuar me contando sobre o que precisa contratar.';
        }
      }
    ...
    .addNode('generate_document', async (state: TrAgentStateType) => {
      const ragResults = await ragService.search('Termo de Referência Lei 14.133 requisitos obrigatórios', 'TR', 8);
